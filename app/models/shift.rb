class Shift < ApplicationRecord
	...
	def is_valid_day_and_service
		...
		is_valid_day = (Shift.where(id: self.service_id, day: self.day)).nil?
		errors.add(:day, "Service with id #{self.service_id} has already a shift on #{self.day}") unless is_valid_day
	end
end
